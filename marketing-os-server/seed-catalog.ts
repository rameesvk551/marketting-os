...
async function seed() {
    try {
        ...
        const tenantInfo = await db.Tenant.findOne();
        if (!tenantInfo) throw new Error('No tenant found');

        const tenantId = tenantInfo.id;

        const user = await db.User.findOne({ where: { tenantId } });
        if (!user || !user.metaAccessToken) throw new Error('No user or metaAccessToken found');
        const ACCESS_TOKEN = user.metaAccessToken;
